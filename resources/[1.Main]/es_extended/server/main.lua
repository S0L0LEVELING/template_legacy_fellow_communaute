...
AddEventHandler('esx:setDuty', function(bool, selected)
    local xPlayer = ESX.GetPlayerFromId(source)
    local eventName
    
    assert(selected == "job" or selected == "faction", "[ERROR] : Un script qui se sert de l'event 'esx:setDuty' n'a pas était mise à jour suite au modification, l'argument 2 n'a pas de type prédifinis ('job' ou 'faction')")
    
    if selected == 'job' then
        eventName = 'esx:setJob'
    elseif selected == 'faction' then
        eventName = 'esx:setFaction'
    end

    if xPlayer[selected].onDuty == bool then
        return
    end
    ...
    TriggerClientEvent(eventName, xPlayer.source, xPlayer[selected])
end)